...
const checkRequiredProps = (props, propName, componentName) => {
  if (!props.dismiss && !props.refresh) {
    return new Error(
      `One of 'dismiss' or 'refresh' is required by '${componentName}' component.`
    );
  }
};
  ...
  render() {
    return (
      <div ref={this.setRef} className="card-tool float-right">
        {this.props.refresh && (
          <em onClick={this.handleRefresh} className="fas fa-sync"></em>
        )}
        {this.props.dismiss && (
          <em onClick={this.handleDismiss} className="fa fa-times"></em>
        )}
      </div>
    );
  }
